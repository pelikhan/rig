...
function bad(path: string, expected: string, actual: unknown): ValidationResult {
  ...
  const isPrimitive = actual === null || typeof actual === "string" || typeof actual === "number" || typeof actual === "boolean";
  const actualDetail = isPrimitive ? ` (${JSON.stringify(actual)})` : "";
  return { ok: false, error: `${path}: expected ${expected}, got ${actualType}${actualDetail}` };
}
