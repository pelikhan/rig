...
| `maxTurns`, `repair` | Explicit retry + repair loop in app logic | Retry/repair policies in agent workflow/harness |
...
| 9 | Schema-repairing extractor | Needs robust retry when output is invalid or partial. | `maxTurns/repair` maps to explicit app-level validation/repair loop. |
